...
import numpy as np
from authentication_handler import create_creds, clear_creds
from get_prediction import predict_json
from env import CLOUD_PROJECT, MODEL_NAME, VERSION_NAME, REGION
...
def get_result(src):
    ...
    img = preprocess(img)
    create_creds()
    response = predict_json(CLOUD_PROJECT, REGION, MODEL_NAME, img, VERSION_NAME)[0]
    clear_creds()
    prob = max(response)
    category = CLASS_NAMES[response.index(prob)]
    result = {
        "pred": category,
        "pred_probability": f"{int(prob * 100)}"
    }
    return result
